...
*Every rule application begins by enumerating every occurrence of a pattern graph
inside the host graph: subgraph isomorphism, NP-complete, and the only
...
then measures each hook by source-patching it off. Two earn their place:
refinement (1.8x on the bench workload, 10.6x on a five-node pattern) and the
node-label pre-filter (7-17x, but only on patterns carrying node labels, which
most of this repo's primitives do not). Two do not, at the sizes measured: the
specialised clone and the `limit`/`delay` triage both fall inside noise. It ends
with a cost model rule authors can act on, and what is not implemented.*

## 1. The problem

The host is a directed `graphlib.Graph` with arbitrary JSON labels. A rule's
...
the `Matcher` language of `$and`, `$or`, `$not`, `$test`, `$find`, `$equals`,
and bare strings read as anchored regexes (`index.js:874`).
...
Nothing constrains host edges *between* images the pattern does not mention, so
this is a **monomorphism** search, not an induced-subgraph or graph isomorphism
search: `a --path--> b` matches a pair of rooms also joined by three other edges.
That permissiveness is the default because dungeon graphs are dense with
backtrack edges, and an induced reading would make almost every primitive
unfireable after a few rewrites. §3(f) covers the post-filters that narrow it.

The search returns **all** matches, not the first, forced by the sampler.
`Context#sampleRuleSite` (`index.js:576`) collects one *site* per (rule,
isomorphism) pair, evaluates a weight per site, and samples proportionally:
...
Truncating enumeration would replace that distribution with whatever the search
order happens to be, and would do it *silently*: the generator would still emit
valid graphs, no test would fail, and the rule weights of
[budgets](budgets.md) would quietly stop meaning what they say. Enumeration is a
correctness requirement, not a convenience.

## 2. The algorithm

`SubgraphSearch` (`subgraph.js:21`) is Ullmann (1976): a table of candidate sets,
an arc-consistency refinement of it, and a backtracking search alternating the
two.

The table `D` is `possibleAssignments`, shaped `{ subnodeId: { hostNodeId: true } }`.
`mapping` is the partial map: `assign` (pattern id → host id), `label` (pattern
id → host label), `match` (pattern id → the record `labelMatch` returned, holding
regex capture groups). Only `assign` drives the search; the other two are
threaded through so RHS expressions like `'${a.match.nodeId[1]}'`
(`dungeon-primitives.js:513`) can read captures out of the LHS match.
...
search instance, written before recursing and restored after, so the accept point
must snapshot it. And each branch clones `D`, which is why cloning is hot at all.
...
| d | compiled-predicate caches | `index.js:765-818` | no | not isolated |
...
type-compatible host nodes rather than `|V_G|`. Note what it misses: **edge**
labels are never pre-filtered. `nodeLabelMatch` and `edgeLabelMatch` both default
to the `labelMatch` `sampleRuleSite` passes in (`subgraph.js:28-30`,
`index.js:587`), but only the node one runs here, so a pattern constrained solely
by its edge label — most of `dungeon-primitives.js` — starts with full candidate
sets and gets all its pruning from refinement.

**(b) `clonePA`.** `D` is `{id: {id: true}}` and nothing else, so a two-level
`for...in` copy is correct and lodash's generic `cloneDeep` unnecessary. Isolated,
it is 1.85-1.96x faster per call (§4.4).
...
all the search mutates — it adds and deletes keys, never edits a value in place —
and the values (host labels owned by the graph, fresh match records) are
read-only downstream. A deep copy would duplicate every host label once per
match: 30,983 times on the bench run.
...
`JSON.stringify` plus an `eval` per call. One `Matcher` is shared across every
stage (`index.js:367`), so rules that spell a predicate alike share its compiled
form.
...
array-of-strings LHS sugar sets it on the chain's first and last node,
...
Neither prunes: they reject after the full cost of finding the match is paid,
saving only the rewrite. A constraint expressible as a label query is strictly
cheaper said that way.
...
made by source-patching `subgraph.js` or `index.js` in memory and pre-seeding
`require.cache`; the repo files were not modified. Every variant reproduces
byte-identical output graphs for all seeds (SHA-1 `82c7e4c1b598` at bench
settings) — the check that these are performance hooks and nothing else.
Repeated baseline runs vary by about ±5%, so differences under ~10% are not
claims.
...
dungeon (10 seeds): median=112.4ms min=69.0ms max=169.9ms avgNodes=22.8 avgEdges=45.7
...
the same deterministic workload, so they are exact.
...
disabling refinement outright — because it paid the loop overhead (an
`Object.keys` allocation per pattern node per candidate, two neighbour lookups)
and pruned nothing for it; counts identical to the refinement-off row confirm it
was an exact no-op.
...
unlabelled ones get dearer (9.1 → 13.3 → 14.5 → 16.0 ms) — constraints on a
...
limit 1) they skip **217** of 5,335, and running those searches anyway and
discarding the result — same output graph, verified by digest — costs 7,414ms
against 7,375ms: noise. The saving is proportional to how often rules are
exhausted, which here is under 4%.

### 4.4 Why the specialised clone does not show up

Cloning a table of `s` pattern nodes × `h` candidates, at the 43,257 calls the
bench workload makes, gives:
...
**mean of 7.2 entries per clone** (24.6 at expansion limit 100). Refinement has
already collapsed most candidate sets to a singleton or to nothing before the
recursion clones them. The hook is cheap and correct; it is not where the time
is.
...
totalling 49.6s, so roughly 22.6s, at least **46% of runtime**, sits in the
refinement loop. Optimisation starts there. One tempting target is not one:
...
* **Label the endpoints, not just the edge.** `midpointRoom` labels its `b` node
  ...
  bare; a positive `type` query on both moves it from §4.2's unlabelled column to
  the labelled one. Do not grow the LHS to say what a label could say: a
  four-node unlabelled chain costs 14.5ms per search on a 66-node graph, a
  labelled one 0.55ms.
* **Prefer a node-label query to a `condition`** where the two are equivalent. A
  ...
  prevents work; a node label prevents the branch. Keep `condition` for what
  labels cannot say — `cycleCloseShortcut`'s `'!$$graph.hasEdge($b.id, $a.id)'`
  (`dungeon-primitives.js:501`) is a claim about a host edge between two matched
  nodes, exactly what §1's semantics leave unconstrained.
...
* **No VF2 or VF3.** VF2 generates candidate pairs from the frontier of the
  partial match and never materialises or clones a table. What is here is the
  older, simpler idea.
* **No candidate ordering.** `subnodes[nAssigned]` takes pattern nodes in
  ...
  the smallest `D(i)` next — is the standard first improvement to Ullmann and
  would matter most for the mixed patterns §4.2 shows are dear. Untried.
* **No degree pre-filter.** A host node with fewer neighbours than its pattern
  node can never match — an `O(|V_G|)` test that would strengthen §3(a) for the
  unlabelled patterns where it does nothing today.
* **No incremental matching**, the largest omission. `sampleRuleSite` re-runs
  ...
  sites whose image meets `Δ`, re-searching within the pattern's radius of `Δ`.
  The cost is not the diffing: `Context#addNode` mints a fresh id for every RHS
  node, so a rewrite changes the identity of the nodes it touches even when their
  labels do not, and a stored site cannot be repaired, only discarded. It would
  also have to preserve §1's enumeration order, or accept that seeds stop
  reproducing across the change.
...
2. **Pre-filter on edge labels.** Seed `D` by also requiring, for each pattern
   edge `(i,k)`, that `j ∈ D(i)` have an outgoing host edge satisfying `λ_P(i,k)`
   — the first refinement sweep hoisted into the constructor, and the missing half
   of §3(a) for primitives that constrain edges rather than nodes.
...
4. **Memoise the label match, not just the compiled predicate.** `labelMatch`
   ...
   one. A memo keyed by (host id, pattern id) would cut into §4.4's 46%.
5. **Give `test/bench.js` a labelled-pattern workload**, and measure §3(d)'s
   caches by patching them off as §4.1 does the others. Every number in §4.1 is
   blind to the pre-filter because the bench grammar uses no node labels, and the
   caches are the one hook in §3's table with no number beside them.
6. **Decide whether `strict` belongs inside the search.** A degree constraint is
...
* [budgets](budgets.md) — `limit`, `delay` and `countType`, the fields §3(e)
  triages on.
* [key-lock-transformations](key-lock-transformations.md) — the rules whose LHS
  shapes §5 costs out.
* [narrative-slots](narrative-slots.md) — what §2's `match` records are read for.
* [murder-mystery](murder-mystery.md), [logic-minigames](logic-minigames.md) —
  rule sets with larger patterns, and so more exposed to §5.
* Source: [`subgraph.js`](../subgraph.js) (all 165 lines);
  [`index.js`](../index.js) `Context#sampleRuleSite` (576-624) and `Matcher`
  (739-1040); [`dungeon-primitives.js`](../dungeon-primitives.js) for LHS shapes;
  [`test/subgraph.test.js`](../test/subgraph.test.js),
